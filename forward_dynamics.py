...
class ForwardDynamics(object):
    ...
    def _init(self, emb_space, ac_space):
        ...
        self.forwardloss = 0.5 * tf.reduce_mean(tf.square(tf.subtract(f, self.phi2)), name='forwardloss')
        self.forwardloss = self.forwardloss * 512.0  # lenFeatures=288. Factored out to make hyperparams not depend on it.

        self.train_step = tf.train.GradientDescentOptimizer(self.learning_rate).minimize(self.forwardloss, var_list=self.get_trainable_variables())
